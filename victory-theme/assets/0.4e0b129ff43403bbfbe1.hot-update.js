...
$(document).ready(function () {
    ...
    function responsiveResize() {
        ...
        if (!$('.active').hasClass("fake_slide")) {
            if ($('.active').hasClass('first_slide')) {
                $("#content1").css('margin-top', '0px');
            } else if ($('.active').hasClass("fake_slide_footer")) {
                $("#content1").css('margin-top', -Math.round(slideIncrement * (currentContent - 2)));
            } else {
                $("#content1").css('margin-top', -Math.round(slideIncrement * (currentContent - 1)));
            }
        }
    ...
    function mainScroll(e) {
        ...
        if (delta < 0 && stop === false && currentContent >= 0 && currentContent < contentNumber) {
            stop = true;
            slideOffset = -parseInt($("#content1").css('margin-top')) + slideIncrement;
            ...
            if (currentContent == 0) {
                ...
                $('#content' + currentContent).removeClass('active');
                currentContent++;
                $('#content' + currentContent).addClass('active');
    ...
    function navScroll() {
        ...
        if (stop === false) {
            ...
            if (navitemScroll < currentNav && navitemScroll != 0 && currentNav != 0) {
                ...
                navSlideOffset = Math.abs(parseInt($("#content1").css('margin-top'))) + slideIncrement * navDiff;
                $("#content1").stop().velocity({
                    marginTop: -Math.round(navSlideOffset)
                }, 850 * navDiff);
                ...
                if (currentNav == 1 || 3) {
                    $(".forward_nav").addClass('black_ver');
                }
                if (currentNav == 5) {
                    $(".forward_nav").animate({
                        marginTop: 0
            ...
            $('#content' + navitemScroll).addClass('active');
        }
    }
    ...
    $('body').swipe({
        swipeUp: function swipeUp(event, direction, distance, duration, fingerCount) {
            swiped = true;
            swipedUp = true;
            swipedDown = false;
            mainScroll();
        },
        swipeDown: function swipeDown(event, direction, distance, duration, fingerCount) {
            swiped = true;
            swipedUp = false;
            swipedDown = true;
            mainScroll();
